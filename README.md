...
- `build.py` — `~/apps/gwangju-index/work/stats/`에서 CSV·지면을 읽어 `site/`를 만든다 (정제 없음, 열 이름만 한글화, png→1600px 그레이 JPEG)
- `site/` — GitHub Pages 루트. index(안내·고지·다운로드·열 설명·공개 도구) / viewer(지면 + 추출 행, x위치 세로선) / data / pages
- 재빌드: `python3 build.py` (이미 변환된 이미지는 건너뜀)
- 로컬 확인: `python3 -m http.server 8791 --directory site`
...
3. 저장소 Settings → Pages → Source: Deploy from a branch, Branch: `main`, Folder: `/site` → Save
